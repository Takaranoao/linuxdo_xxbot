...
fn full_env_round_trip() {
    ...
    assert_eq!(cfg.api_id, 10000);
    assert_eq!(cfg.session_path, "/tmp/tg.session");
    assert_eq!(cfg.target_topic_id, Some(7310786));
    assert_eq!(cfg.target_reply_to_msg_id, Some(98765));
    let url = build_proxy_url(
        cfg.proxy_type.as_deref(),
        cfg.proxy_host.as_deref(),
        cfg.proxy_username.as_deref(),
        cfg.proxy_password.as_deref(),
    )
    .unwrap();
